...
class ImageSmoothingApp(QWidget):
    def __init__(self):
        ...
        self.median_filter_button.clicked.connect(lambda: self.apply_filter(self.apply_median_filter, size=3))

        self.rect_filter_button_5x5 = QPushButton('Apply Rectangular Filter (5x5)', self)
        self.rect_filter_button_5x5.clicked.connect(lambda: self.apply_filter(self.apply_rectangular_filter, size=5))
    ...
    def apply_gaussian_filter(self, image, sigma):
        ...
        for i in range(image.shape[0]):
            for j in range(image.shape[1]):
                result[i, j] = np.sum(kernel * padded_image[i:i + kernel_size, j:j + kernel_size])
